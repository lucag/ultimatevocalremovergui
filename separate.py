...
class SeperateAttributes:
    def __init__(
        ...
        self.is_save_inst_vocal_splitter = (
            ...
            and model_data.is_save_inst_vocal_splitter
        )
        ...
        self.is_bv_model_rebalenced = (
            model_data.bv_model_rebalance and self.is_vocal_split_model
        )
            ...
            if mps_available:
                ...
                if cuda_available:  # and not self.is_use_opencl:
                    self.device = (
                        ...
                        else f"{device_prefix}:{self.device_set}"
                    )
        ...
        if model_data.process_method == DEMUCS_ARCH_TYPE:
            ...
            self.device = (
                cpu
                if self.is_other_gpu
                and not self.demucs_version in [DEMUCS_V3, DEMUCS_V4]
                else self.device
            )
            ...
            if self.is_multi_stem_ensemble and main_model_primary:
                ...
                if main_model_primary in self.demucs_source_map.keys():
                    ...
                    secondary_stem(main_model_primary) in self.demucs_source_map.keys()
                ):
    ...
    def start_inference_console_write(self):
        ...
        if self.is_vocal_split_model:
            self.write_to_console(
                INFERENCE_STEP_2_VOC_S(self.process_method, self.model_basename)
            )
    ...
    def process_secondary_stem(
        self, stem_source, secondary_model_source=None, model_scale=None
    ):
        if not self.is_secondary_model:
            if self.is_secondary_model_activated and isinstance(
                secondary_model_source, np.ndarray
            ):
    ...
    def write_audio(self, stem_path: str, stem_source, samplerate, stem_name=None):
        ...
        def save_voc_split_instrumental(stem_name, stem_source, is_inst_invert=False):
            ...
            inst_stem_path = self.audio_file_base_voc_split(
                INST_STEM, inst_stem_path_name
            )
        ...
        def deverb_vocals(stem_path: str, stem_source):
            ...
            save_audio_file(
                stem_path.replace(".wav", "_deverbed.wav"), stem_source_deverbed
            )
            save_audio_file(
                stem_path.replace(".wav", "_reverb_only.wav"), stem_source_2
            )
...
class SeperateMDX(SeperateAttributes):
    ...
    def demix(self, mix, is_match_mix=False):
            ...
            with torch.no_grad():
                for mix_wave in mix_waves:
                    self.running_inference_progress_bar(
                        total_chunks, is_match_mix=is_match_mix
                    )
        ...
        if self.is_denoise_model and not is_match_mix:
            if (
                NO_STEM in self.primary_stem_native
                or self.primary_stem_native == INST_STEM
            ):
                ...
                source = vr_denoiser(
                    source, self.device, model_path=self.DENOISER_MODEL
                )
    ...
    def seperate(self):
        ...
        if (
            ...
            for stem in stem_list:
                ...
                self.write_audio(
                    primary_stem_path, self.primary_source, samplerate, stem_name=stem
                )
...
class SeperateDemucs(SeperateAttributes):
    def seperate(self):
        ...
        if (
            ...
            for stem_name, stem_value in self.demucs_source_map.items():
                ...
                self.write_audio(
                    stem_path, stem_source, samplerate, stem_name=stem_name
                )
    ...
    def demix_demucs(self, mix):
        ...
        if self.is_pitch_change:
            sources = np.stack(
                [self.pitch_fix(stem, sr_pitched, org_mix) for stem in sources]
            )
...
class SeperateVR(SeperateAttributes):
    ...
    def loading_mix(self):
        ...
        audio_file = spec_utils.write_array_to_mem(
            self.audio_file, subtype=self.wav_type_set
        )
...
def vr_denoiser(
    ...
    if is_deverber:
        ...
        mp = ModelParameters(
            os.path.join("lib_v5", "vr_network", "modelparams", "4band_v3.json")
        )
